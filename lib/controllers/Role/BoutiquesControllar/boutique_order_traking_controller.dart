...
class BoutiqueOrderTrackingControllar extends GetxController {
  final Completer<GoogleMapController> mapController =
      Completer<GoogleMapController>();

  Rx<BoutiqueTrackModel> boutiqueTrackModel = BoutiqueTrackModel().obs;
...
/// Order Track Details
  ...
  boutiqueOrderTrack(String orderId) async {
    orderTrackLoading(true);
    var response = await ApiClient.getData(
        '${ApiConstant.boutiqueOrderTrackEndPoint}/$orderId');
    if (response.statusCode == 200) {
      boutiqueTrackModel.value =
          BoutiqueTrackModel.fromJson(response.body['data']['attributes']);
  ...
  listenDriverLocation() async {
    ...
    SocketApi.socket.on('orderStatus', (data) async {
      ...
      orderTextUpdate();
      update();
      debugPrint('Boutique DriverLocationddd>>${driverLocationModel.value.location!.longitude}');
    });
  }
  ...
  void addMarkers() async {
    ...
    final driverMarkerIndex = markerList.indexWhere((marker) => marker.markerId == MarkerId('1'));

    if (driverMarkerIndex != -1) {
      print("Driver Lat${driverLocationModel.value.location!.latitude!}");
  ...
  /// Convert Delivery Address to Lat Long
    ...
    List locations = await locationFromAddress(boutiqueTrackModel.value.deliveryAddress!);
    if (locations.isNotEmpty) {
      ...
      print('DriverDeliveryAddress: ${boutiqueTrackModel.value.deliveryAddress!}');
